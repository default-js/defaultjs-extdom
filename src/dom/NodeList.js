...
NodeList.prototype.val = function () {
	if (arguments.length == 0) {
		...
		return result;
	} else NodeList.prototype.applyTo.apply(this, ["val"].concat(Array.from(arguments)));
...
NodeList.from = function () {
	...
	while (args.length > 0) {
		...
		if (typeof arg !== "undefined" && arg != null) {
			if (arg instanceof Node) data[counter++] = { value: arg, enumerable: true };
	...
	function (aFunctionName, theArguments) {
		...
		this.forEach((node) => {
			if (node && typeof node[aFunctionName] === "function") {
				...
				if (result != null) {
					if (result instanceof NodeList) results = results.concat(Array.from(result));
